...
public class TestCheckStand {
    public static void main(String[] args) {
        ...
        Client.run();
        //运行结束后，将订单清空
        OrderManage.orders.clear();
        try {
            OrderManage.orders.store(new FileOutputStream(OrderManage.orderfile), "清空订单");
        } catch (IOException e) {
            e.printStackTrace();
        }
...
class GoodsManage {
    ...
    public static File file = new File(
            "D:\\mycode\\Items\\MyItems\\src\\Arivan\\CheckStand1\\Goods.properties");
    public static Properties goods = new Properties();
        ...
        while (true) {
            ...
            switch (value) {
                ...
                case "U":
                    while (true) {
                        try {
                            goods.load(new FileInputStream(file));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        if (goods.isEmpty()) {
    ...
    public static void view() {
        ...
        if (goods.isEmpty()) {
            ...
            try {
                goods.load(new FileInputStream(file));
                iterator = goods.keySet().iterator();
            } catch (IOException e) {
                e.printStackTrace();
            }
...
//                String[] arr = goods.get(keyValue);
                String[] arr = goods.getProperty(keyValue).trim().split(" ");
        ...
        try {
            ...
            while (true) {
                ...
                if (arr.length != 3) {
                    ...
                    goods.setProperty(arr[0], (arr[1] + " " + arr[2]));
                    goods.store(new FileOutputStream(file), "写入文件");
                    break;
                }
            }
        } catch (IOException e) {
    ...
    public static void downGoods() {
        ...
        goods.remove(input);
        try {
            goods.store(new FileOutputStream(file), "下架商品");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
class OrderManage {
    ...
    public static File orderfile = new File(
            "D:\\mycode\\Items\\MyItems\\src\\Arivan\\CheckStand1\\Orders.properties");
    public static Properties orders = new Properties();
        ...
        while (true) {
            ...
            if (input.equals("R")) {
                ...
                switch (input) {
                    ...
                    case "A":
                        try {
                            GoodsManage.goods.load(new FileInputStream(GoodsManage.file));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        if (GoodsManage.goods.isEmpty()) {
                            ...
                            while (true) {
                                buy();
                                orderView();
                                System.out.println("停止下单，请输入R,继续下单请按回车");
                                if (scanner.nextLine().equals("R")) {
                                    break;
                                }
                            }
                    ...
                    case "D":
                        if (orders.isEmpty()) {
                            ...
                            while (true) {
                                cancel();
                                orderView();
                                System.out.println("返回上一层，请输入R，继续取消请按回车");
                                if (scanner.nextLine().equals("R")) {
                                    break;
                                }
                            }
        ...
        try {
            ...
            while (true) {
                ...
                if (arr.length != 2) {
                    ...
                    if (!GoodsManage.goods.containsKey(arr[0])) {
                        System.out.println("输入有误，商品编号不存在！");
                        continue;
                    }
                    String count = arr[1];
                    String[] good = GoodsManage.goods.getProperty(String.valueOf(id))
                            .trim().split(" ");
                    orders.setProperty(String.valueOf(id), (good[0] + " " + count + " " + good[1]));
//                    orderView();
                    orders.store(new FileOutputStream(orderfile), "输出订单信息");
                    break;
                }
            }
        } catch (IOException e) {
    ...
    public static void orderView() {
        ...
        if (!orders.isEmpty()) {
            Iterator<Object> iterator = orders.keySet().iterator();
            while (iterator.hasNext()) {
                String id = (String) iterator.next();
                String[] value = orders.getProperty(id).trim().split(" ");
                System.out.print(id);
    ...
    public static double ordersSum() {
        ...
        double sum = 0.0;
        while (iterator.hasNext()) {
            String[] value = orders.getProperty((String) iterator.next()).trim().split(" ");
    ...
    public static void cancel() {
        ...
        while (true) {
            ...
            if (GoodsManage.isAllDigit(arr[0]) && GoodsManage.isAllDigit(arr[1])) {
                //取得订单中的商品名称、数量、单价
                String[] value = orders.getProperty(arr[0]).trim().split(" ");
                //取得订单中的商品数量
                ...
                if (realcount <= count || count < 0) {
                    ...
                    String newValue = value[0] + " " + value[1] + " " + value[2];
                    orders.setProperty(arr[0],newValue);
                }
                try {
                    orders.store(new FileOutputStream(orderfile),"修改订单");
                } catch (IOException e) {
                    e.printStackTrace();
                }
                break;
